...
def showprofileadmin(usershow) :
    ...
    Label(adminframe,text='Type',bg='skyblue',fg='white').grid(row=3,column=4,sticky='wes')
...
def history() :
    ...
    mytree1 = ttk.Treeview(adminframeh, columns=('username','egg','cucumber','onion'),height=2)
    
    mytree1.heading('#0',text='')
    mytree1.heading('username',text='Receipt Number',anchor=CENTER)
    mytree1.heading('egg',text='Emp_id',anchor=CENTER)
    mytree1.heading('cucumber',text='Oder details',anchor=CENTER)
    mytree1.heading('onion',text='Total',anchor=CENTER)
    ...
    mytree1.column('username',anchor=CENTER,width=150)
    mytree1.column('egg',anchor=CENTER,width=90)
    mytree1.column('cucumber',anchor=CENTER,width=700)
    ...
    tree_scrollbar_h.grid(row=2,column=4,sticky='nsw')
...
def fetch_tree1():
    ...
    if result:
        for i,data in enumerate(result):
            mytree1.insert('','end',values=(data[0],data[1],data[2],data[3]))


def fetch_search1():
    mytree1.delete(*mytree1.get_children())
    sql = 'select * from receipt WHERE rec_id=?'
...
def checkout():
    ...
    Label(pickframe,text="Thank you for coming \n we are very happy that you came",bg='white').grid(row=1,column=0)
